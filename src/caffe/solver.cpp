  ...
  if (lr_policy == "fixed") {
    ...
    rate = this->param_.base_lr() *
        pow(Dtype(1) + this->param_.gamma() * this->iter_,
            - this->param_.power());
  } else if (lr_policy == "linear") {
    int current_epoch = this->iter_ / this->param_.epoch_size();
    rate = this->param_.base_lr() - this->param_.base_lr() * ((float)current_epoch / (float)this->param_.epoch_size());
  } else {
